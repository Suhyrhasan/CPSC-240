...
;Assemble: nasm -f elf64 -o view.o -l view.lis viewstack.asm

;Parameters passed in:
;     rdi = identifying integer
;     rsi = number of qwords below and outside the top of stack, a positive integer
...
;Output the header before outputting memory
mov rax, 0
mov rdi, stackheadformat
mov rsi, [rsp+13*8]    ;rsi hold the identifying integer passed here from the caller function
mov rdx, [rsp+14*8]    ;rdx hold the base address of the caller procedure
mov rcx, rsp
add rcx, 128            ;rdx hold the top of stack pointer when the caller invoked this procedure
call printf
...
add r12,128 ;120                                                 ;Add the distance between current rsp and the "return address"
...
;
;Pick a number to send back to the calling program.  We will send the address of the top of stack back to the caller
mov rax,rsp
